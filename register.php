...
if (isset($_POST['form_sent']))
{
	...
	$realname = pun_trim($_POST['realname']);
	...
	if (empty($errors))
	{
		...
		// Add the user
		$db->query('INSERT INTO '.$db->prefix.'users (username, realname, group_id, password, email, email_setting, timezone, dst, language, style, registered, registration_ip, last_visit) VALUES(\''.$db->escape($username).'\', \''.$db->escape($realname).'\', '.$intial_group_id.', \''.$password_hash.'\', \''.$db->escape($email1).'\', '.$email_setting.', '.$timezone.' , '.$dst.', \''.$db->escape($language).'\', \''.$pun_config['o_default_style'].'\', '.$now.', \''.get_remote_address().'\', '.$now.')') or error('Unable to create user', __FILE__, __LINE__, $db->error());
...
$required_fields = array('req_user' => $lang_common['Username'], 'realname' => $lang_common['realname'], 'req_password1' => $lang_common['Password'], 'req_password2' => $lang_prof_reg['Confirm pass'], 'req_email1' => $lang_common['Email'], 'req_email2' => $lang_common['Email'].' 2');
...
<?php endif; ?><div class="inform">
				<fieldset>
					<legend><?php echo $lang_register['realname legend'] ?></legend>
					<div class="infldset">
						<input type="hidden" name="form_sent" value="1" />
						<label class="required"><strong><?php echo $lang_common['realname'] ?> <span><?php echo $lang_common['Required'] ?></span></strong><br /><input type="text" name="realname" value="<?php if (isset($_POST['realname'])) echo pun_htmlspecialchars($_POST['realname']); ?>" size="25" maxlength="25" /><br /></label>
					</div>
				</fieldset>
			</div>
						<div class="inform">
